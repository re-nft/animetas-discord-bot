...
async def post_connect():
    ...
    try:
        discord_user = get_discord_user(body["guildId"], body["userId"], address)
        add_guild_id(body["guildId"])
        add_user(body["guildId"], body["userId"], address)
        guild_name = discord_user.guild.name
    except Exception as e:
        err_body = {"success": False, "message": f"{e}"}
        return Response(json.dumps(err_body), status=500, mimetype="application/json")

    if len(discord_user.proposed_roles) > 0:
        await assign_roles(discord_user.member, list(discord_user.proposed_roles))

        title = "Wallet Connected"
        body = (
            "Wallet is connected. "
            "Please check assigned roles in "
            f"**{guild_name}**. "
        ...
        return {"success": False, "message": "You do not own or have rented an NFT"}
